...
# Load the ssGSEA results
Both of these results files are based on the fasting mimicking diet paper by Vernieri et al. (2022).

The first set of results predict immune cell presence using gene signatures from Charoentong et al. The same method was used by Vernieri et al. (2022) in their analysis.

The second set of results looks at enrichment of the immune related gene signatures also used by Vernieri et al. (2022) in their analysis. They appear to have collected this immune gene signature list themselves.

```{r load_files}

# Read in the Charoentong signature immune cell type results
cell_types <- read.csv(file = paste(data_directory, "Charoentong_results.csv", sep = "/"), row.names = 1, check.names = FALSE)

# Read in the Charoentong immune associated signature results
immune_sigs <- read.csv(file = paste(data_directory, "Vernieri_gene_signatures_results.csv", sep = "/"), row.names = 1, check.names = FALSE)

# Read in the metadata
metadata <- readRDS(file = paste(data_directory, "metadata.rds", sep = "/"))
metadata <- metadata[metadata$WINGMEN.sampleID %in% colnames(cell_types),]
...
# Transpose the data
cell_types <- as.data.frame(t(cell_types))
immune_sigs <- as.data.frame(t(immune_sigs))

# Add treatment and patient information from metadata to the results objects
if(all(rownames(cell_types) == metadata$WINGMEN.sampleID)){
  cell_types$WINGMEN.patientID <- metadata$WINGMEN.patientID
  cell_types$WINGMEN.sampleID <- metadata$WINGMEN.sampleID
  cell_types$Treatment <- metadata$Treatment
}

if(all(rownames(immune_sigs) == metadata$WINGMEN.sampleID)){
  immune_sigs$WINGMEN.patientID <- metadata$WINGMEN.patientID
  immune_sigs$WINGMEN.sampleID <- metadata$WINGMEN.sampleID
  immune_sigs$Treatment <- metadata$Treatment
}
...
# Rename
colnames(cell_types)[3] <- "Activated CD8+ T cell"
colnames(cell_types)[8] <- "Central memory CD8+ T cell"
colnames(cell_types)[10] <- "Effector memory CD8+ T cell"
colnames(cell_types)[25] <- "Tissue resident memory CD8+ T cell"

# You can type your cell types of interest using the format below 
my_cells <- c("Activated CD8+ T cell",
...
for(i in my_cells){
  ...
  p1[[i]] <- ggplot(cell_types, 
                    aes(Treatment, get(i), fill = Treatment, text = WINGMEN.sampleID)) + 
             geom_boxplot()+ 
             geom_point()+
             ylab(my_title)+
             scale_fill_manual(values = c("black", "#f89b7c"))+
             #ggtitle("Your title")+
             theme(
  ...
  for(patient in metadata$WINGMEN.patientID[duplicated(metadata$WINGMEN.patientID)]){
      p1[[i]] <- p1[[i]]+geom_segment(data = cell_types[cell_types$WINGMEN.patientID == patient,], 
  ...
  temp.df <- cell_types[, c("WINGMEN.sampleID", i)]
...
plotly::config(p1$`Tissue resident memory CD8+ T cell`,
    toImageButtonOptions = list(
      format = "png", # one of png, svg, jpeg, webp
      filename = "Tissue resident memory CD8 T cell",
      width = 300,
      height = 350,
      scale = 5
    ))
...
for(i in my_immune_sigs){
  ...
  p2[[i]] <- p2[[i]] %>% 
                add_annotations(
                  text = paste("Wilcoxon p =", round(wilcox.test(as.numeric(temp.df$PRE), temp.df$POST, paired = T)$p.value, 3)), 
                  x = 1.5,  
                  y = 1.05,
                  xref = "x",
                  yref = "paper",
...
# Save a plot. Here you will need to change the signature in the p2$IL6 to your signature of interest, and the filename. Once you have run the code, it will appear in the viewer. You can then download the plot from the options menu that appears above the plot. Unfortunately I haven't found a way to automate this!
